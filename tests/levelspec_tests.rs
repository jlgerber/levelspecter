...
fn can_parse_shot() {
    ...
    assert_eq!(ls, Ok(LevelSpec::new_shot("DEV01", "RD", "9999")));
}
...
fn can_parse_shot_with_wildcard() {
    ...
    assert_eq!(ls, Ok(LevelSpec::new_shot("DEV01", "%", "9999")));
}

#[test]
fn can_parse_seq() {
    let ls = LevelSpec::from_str("DEV01.RD");
    assert_eq!(ls, Ok(LevelSpec::new_sequence("DEV01", "RD")));
}

#[test]
fn can_parse_show() {
    let ls = LevelSpec::from_str("DEV01");
    assert_eq!(ls, Ok(LevelSpec::new_show("DEV01")));
}